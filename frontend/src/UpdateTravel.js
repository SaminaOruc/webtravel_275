...
function UpdateTravel(){
    ...
    const handleSubmit=(event)=>{
        ...
        if(errors.title==="" && errors.description===""){
            axios.put("http://localhost:8081/admin/update-travel/"+id, values)
            .then(res=>{
                console.log("Server response:", res.data);
                alert("Travel has been successfully updated");
                navigate("/admin");
            })
        ...
        <div className="d-flex justify-content-center align-items-center bg-warning vh-100">
            <div className="bg-white p-3 rounded w-25">
                ...
                <form action="" onSubmit={handleSubmit}>
                    ...
                    <button type="submit" className="btn btn-success w-100">Update Travel</button>
                </form>
            </div>
        </div>
    )
